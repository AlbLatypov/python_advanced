...
with open('logfile.txt', 'r') as file:
    for lines in file:
        user, t1, t2 = lines.split(',')
        h1,m1=t1.strip().split(':')
        h2,m2=t2.strip().split(':')
        total = (int(h2)*60+int(m2)) - (int(h1)*60+int(m1))
        print(f'total time - {total}')
        print(user,total)
        with open('output122.txt', 'a', encoding='UTF-8') as out_file:
            if total>60:
                print(user,file=out_file)
