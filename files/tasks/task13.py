...
Pink goat
'''
# 1. count - общее количество козлов
# 2. Количество козлов каждого цвета при вводе - Создать словарь с ключом цвет, значение - пусто
# 3. Вывести, если полученное количество более 7% от общего количества.
...
with open('goats.txt', 'r') as input_file:
    ...
    for line in input_file:
        color, _ = line.split()
        goats[color] = goats.get(color, 0) + 1
        total += 1

with open('answer.txt', 'w') as output_file:
    for key, val in goats.items():
        if val > total * 0.07:
            print(key, 'goat', file=output_file)
